...
volatile float64 master_position, slave_position, theta_rad, theta_deg, theta_error;
...
void main(void) {
  ...
  //EQep1Regs.QPOSCNT = (Uint32)(ENCODER_OFFSET);
...
__interrupt void cpu_timer0_isr(void)  // TIMER0 INTERRUPT ROUTINE
{
  ...
  position_error = master_position - slave_position;
  ...
  theta_ref = c_gain*(position_error - (z_0+z_1) * position_error_1 + (z_0*z_1) * position_error_2) + (p_0+p_1) * theta_ref_1 - (p_0*p_1) * theta_ref_2;
  //*/
  ...
  //float64 p_0 = 0.3652;
  //float64 z_0 = .965;
  //float64 c_gain = 121.1;

  // PD
  //float64 p_0 = 0;
  //float64 z_0 = .9639;
  //float64 c_gain = 196.3;

  //theta_ref = c_gain * (position_error - (z_0)*position_error_1) + (p_0)*theta_ref_1;
  //*/
