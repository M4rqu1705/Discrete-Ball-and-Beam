...
#define ADC_SENSITIVITY (0.42 / 4096.0)
#define ENCODER_SENSITIVITY (-PI / 2048.0)

volatile float64 adc_master = 0, adc_slave = 0, encoder = 0, motor_volts = 0;
volatile float64 master_position = 0, slave_position = 0, angle_rad = 0,
                 angle_deg = 0;
...
__interrupt void cpu_timer0_isr(void)  // TIMER0 INTERRUPT ROUTINE
{
  ...
  angle_rad = encoder * ENCODER_SENSITIVITY;
